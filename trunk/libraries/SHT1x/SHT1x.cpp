...
float SHT1x::readHumidity()
{
  ...
  _temperatureC = readTemperatureC();

  // Correct humidity value for current temperature
  _correctedHumidity = _temperatureC * (T1 + T2 * _val) + _linearHumidity;
...
float SHT1x::update()
{
	readHumidity(); //call to this function does both because of correction
}

/**
 * Get temperature after update
 */
float SHT1x::getTemperatureC()
{
	return (_temperatureC); 
}
...
{ 
	float logEx,dew_point;
 	logEx=0.66077+7.5*_temperatureC/(237.3+_temperatureC)+(log10(_correctedHumidity)-2);
